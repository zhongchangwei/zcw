...
<body class="body1">
	<div class="global2">
		<div class="datagridtitle">部门管理</div>
		<div class="mini-toolbar">
		部门名称：<input id="rName" class="mini-textbox" maxlength="20" style="width:90px" />
		...
		<a id="add" class="mini-button mini-button-info" onclick="edit">修改</a>
		</div>
	</div>
	<div class="mini-fit">
	...
	<div property="columns">
		...
		<div field="DEPT_NAME" headerAlign="center" width="90">部门名称</div>
...
<script type="text/javascript">
    ...
    var where ="";
    if('${pid}'!=''){
    	where = " start with u.id='${pid}' connect by prior u.id=pid";
    }
	 var replace = {"##replace##":where};//" and u.pid = '"+${pid}+"'"
	...
	function search(){
		var rname= mini.get("rName").getValue();	
		 var tem="";
		 if(rname!=""&&rname!=null){
			 tem+=" and U.DEPT_NAME like '%"+rname+"%'";
		 }
    ...
    function add(){
    	var rows = grid.data;
    	mini.open({
    		url:"<%=path%>/deptController/openDetail.do?pid=${pid}",
    		title:"案件接收新增",
    		showMaxButton:true,
  ...
  //弹出修改窗口
    function edit(){
    	...
    	mini.open({
    		url:"<%=path%>/deptController/openDetail.do?pid=${pid}&id="+rows[0].PK,
    		title:"案件接收修改",
    		showMaxButton:true,
    		height:618,width:1000,
    		ondestroy:function(action) {
