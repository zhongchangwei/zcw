...
@Service("codeMenuService")
public class CodeMenuServiceImpl implements CodeMenuService{
	@Autowired
	...
	public String list() throws Exception {
		String sql=new BaseScanSqlCommand().getSqlText(this, "codeMenuSql.xml", "list");
	...
	public String showData(ReceParam rcep) throws Exception {
		String sql=new BaseScanSqlCommand().getSqlText(this, "codeMenuSql.xml", "showFormData");
	...
	public String save(ReceParam rcep) throws Exception {
		...
		if(StringUtils.isEmpty(rcep.getForms())){
			...
			//新增单个
			TCodeInfo beanAdd = rcep.getBeanAdd(bean, rcep.getForms());
			id = beanAdd.getId();
		...
		if(StringUtils.isNotEmpty(rcep.getAddRows())){
			//批量新增
			List<TCodeInfo> listBeanAdd = rcep.getListBeanAdd(bean, rcep.getAddRows());
